	...
	integrations: [
		...
		sitemap({
			...
			serialize(item) {
				...
				if (
					url === "https://metabolic40plus.com" ||
					url === "https://metabolic40plus.com/"
				) {
					item.priority = 1.0;
					item.changefreq = "daily";
				} else if (
					url.includes("/coffee-wellness") ||
					url.includes("/sleep-recovery")
				) {
					if (url.match(/\/coffee-wellness\/coffee-metabolism\/.+/)) {
						...
						url.match(/\/coffee-wellness$/) ||
						url.match(/\/sleep-recovery$/)
					) {
						item.priority = 0.8;
						item.changefreq = "weekly";
					} else {
